...
function addStopButtons(){
  document.querySelectorAll(".stopCard .stopActions").forEach(actions=>{
    ...
    if(!google)return;
    const b=document.createElement("button");b.className="btn cc-waze-stop";b.textContent="Waze";
    b.onclick=()=>{const c=coordFromGoogleButton(google);if(c)openWazeCoord(c)};
...
function openLeg(leg){if(!leg)return;leg.type==="coord"?openWazeCoord(leg.coord):openWazeSearch(leg.query)}
function startFullTrip(){const legs=itinerary();if(!legs.length)return;writeProgress({sig:signature(legs),index:0,total:legs.length,startedAt:Date.now()});renderFullTripPanel(true);openLeg(legs[0])}
function openNextLeg(){const legs=itinerary();if(!legs.length)return;const sig=signature(legs),p=readProgress();let next=0;if(p&&p.sig===sig)next=Math.min(p.index+1,legs.length-1);writeProgress({sig,index:next,total:legs.length,startedAt:p?.startedAt||Date.now()});renderFullTripPanel(true);openLeg(legs[next])}
function reopenCurrentLeg(){const legs=itinerary();if(!legs.length)return;const sig=signature(legs),p=readProgress(),i=p&&p.sig===sig?Math.min(p.index,legs.length-1):0;writeProgress({sig,index:i,total:legs.length,startedAt:p?.startedAt||Date.now()});renderFullTripPanel(true);openLeg(legs[i])}
function renderFullTripPanel(force=false){
  const card=$("routeSummaryCard");if(!card)return;
  const legs=itinerary();let panel=$("wazeFullTripPanel");
  if(!legs.length){panel?.remove();return}
  const sig=signature(legs),p=readProgress();
  if(p&&p.sig!==sig)clearProgress();
  const pp=readProgress(),active=pp&&pp.sig===sig?Math.min(pp.index,legs.length-1):-1;
  const key=`${sig}|${active}`;
  if(!panel){panel=document.createElement("div");panel.id="wazeFullTripPanel";panel.style.cssText="margin-top:12px;padding:12px;border:1px solid #d0d5dd;border-radius:7px;background:#f8fafc";card.appendChild(panel)}
  if(!force&&panel.dataset.key===key)return;
...
function addRouteButton(){
  const save=$("saveRouteBtn");if(!save)return;
  let b=$("launchWazeRouteBtn");
  if(!b){b=document.createElement("button");b.id="launchWazeRouteBtn";b.className="btn primary";save.insertAdjacentElement("afterend",b)}
  b.textContent="Trajet complet Waze";b.onclick=startFullTrip;
}
let scheduled=false;
function refresh(){
  if(scheduled)return;scheduled=true;
  requestAnimationFrame(()=>{scheduled=false;addStationButton();addStopButtons();addRouteButton();renderFullTripPanel()});
}
const stops=$("routeStops");if(stops)new MutationObserver(refresh).observe(stops,{childList:true,subtree:true});
const summary=$("routeSummaryCard");if(summary)new MutationObserver(refresh).observe(summary,{attributes:true,attributeFilter:["class"]});
document.querySelectorAll(".tabs button").forEach(b=>b.addEventListener("click",()=>setTimeout(refresh,40)));
$("planRouteBtn")?.addEventListener("click",()=>setTimeout(refresh,100));
